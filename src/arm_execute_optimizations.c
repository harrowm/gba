...
// Define rotation macro if it doesn't exist
#ifndef ARM_ROTATE_RIGHT
#define ARM_ROTATE_RIGHT(value, shift) (((value) >> (shift)) | ((value) << (32 - (shift))))
#endif

// Function pointer tables
ArmConditionChecker arm_condition_table[16]; // Using ArmConditionChecker from header
ArmInstructionHandler arm_instruction_table[16]; // Using ArmInstructionHandler from header
ALUOperation arm_alu_operation_table[16];

// Forward declarations for optimized execution functions
...
static bool arm_handle_data_processing(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
static bool arm_handle_multiply(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
static bool arm_handle_single_data_transfer(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
static bool arm_handle_block_data_transfer(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
static bool arm_handle_branch(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
static bool arm_handle_coprocessor(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
static bool arm_handle_software_interrupt(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
static bool arm_handle_undefined(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface);
...
static bool arm_handle_multiply(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface) {
    UNUSED(memory_interface); // Cast to void to suppress "unused parameter" warning
...
static bool arm_check_condition_al(uint32_t cpsr) {
    UNUSED(cpsr); // Not needed for AL (always) condition
    return true; // Always
}

static bool arm_check_condition_nv(uint32_t cpsr) {
    UNUSED(cpsr); // Not needed for NV (never) condition
    return true; // Never (treated as always in ARMv4)
}

// Individual ALU operation functions
/* This function appears to be a wrapper with different signature than used in the ALU table */
__attribute__((unused))
static uint32_t arm_alu_and_wrapper(ArmCPUState* state, uint32_t op1, uint32_t op2, uint32_t* carry_out, bool set_flags) {
    UNUSED(set_flags); // Handled inside the function
...
static uint32_t arm_alu_sub(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // Not used directly in SUB
    uint32_t result = op1 - op2;
    bool set_flags = ARM_GET_S_BIT(state->cpsr);
...
static uint32_t arm_alu_rsb(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // Not used directly in RSB
...
static uint32_t arm_alu_add(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // Not used directly in ADD
    uint32_t result = op1 + op2;
    bool set_flags = ARM_GET_S_BIT(state->cpsr);
...
static uint32_t arm_alu_adc(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // Not used directly in ADC
...
static uint32_t arm_alu_sbc(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // Not used directly in SBC
    uint32_t carry_in = (state->cpsr & ARM_FLAG_C) ? 1 : 0;
...
static uint32_t arm_alu_rsc(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // Not used directly in RSC
    uint32_t carry_in = (state->cpsr & ARM_FLAG_C) ? 1 : 0;
...
static uint32_t arm_alu_tst(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    ...
    return result; // Result not stored (but we return it anyway to avoid warnings)
}
...
static uint32_t arm_alu_cmp(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // carry_out is not used in CMP
    uint32_t result = op1 - op2;
...
static uint32_t arm_alu_cmn(uint32_t op1, uint32_t op2, uint32_t* carry_out, ArmCPUState* state) {
    UNUSED(carry_out); // carry_out is not used in CMN
    uint32_t result = op1 + op2;
 ...
 */
bool arm_execute_data_processing_optimized(ArmCPUState* state, uint32_t instruction, ArmMemoryInterface* memory_interface) {
    UNUSED(memory_interface); // Not used directly in data processing
