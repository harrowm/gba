...
protected:
    ...
    ThumbCPUTest15() : memory(true), cpu(memory, interrupts), thumb_cpu(cpu), ks(nullptr) {}
...
TEST_F(ThumbCPUTest15, STMIA_SINGLE_REGISTER) {
    ...
    if (!assemble_and_write_thumb("stmia r0!, {r1}", 0x00000000)) {
        memory.write16(0x00000000, 0xC002); // STMIA R0!, {R1}
    }
    ...
    EXPECT_EQ(cpu.R()[0], 0x00001004u);
    EXPECT_EQ(cpu.R()[15], 0x00000002u);
    
    // Test case 2: STMIA R2!, {R0}
    setup_registers({{2, 0x00001100}, {0, 0x12345678}});
    
    if (!assemble_and_write_thumb("stmia r2!, {r0}", 0x00000002)) {
        memory.write16(0x00000002, 0xC201); // STMIA R2!, {R0}
    }
    ...
    if (!assemble_and_write_thumb("stmia r7!, {r7}", 0x00000004)) {
        memory.write16(0x00000004, 0xC780); // STMIA R7!, {R7}
    }
...
TEST_F(ThumbCPUTest15, STMIA_MULTIPLE_REGISTERS) {
    ...
    if (!assemble_and_write_thumb("stmia r0!, {r0, r1}", 0x00000000)) {
        memory.write16(0x00000000, 0xC003); // STMIA R0!, {R0, R1}
    }
    ...
    // Check that R0 was incremented
    EXPECT_EQ(cpu.R()[0], 0x00001008u);
    EXPECT_EQ(cpu.R()[15], 0x00000002u);
    
    // Test case 2: STMIA R3!, {R0, R2, R4, R6}
    setup_registers({{3, 0x00001300}, {0, 0xAAAAAAAA}, {2, 0xCCCCCCCC}, {4, 0xEEEEEEEE}, {6, 0x66666666}});
    
    if (!assemble_and_write_thumb("stmia r3!, {r0, r2, r4, r6}", 0x00000002)) {
        memory.write16(0x00000002, 0xC355); // STMIA R3!, {R0, R2, R4, R6}
    }
    ...
    if (!assemble_and_write_thumb("stmia r1!, {r0, r1, r2, r3, r4, r5, r6, r7}", 0x00000004)) {
        memory.write16(0x00000004, 0xC1FF); // STMIA R1!, {R0-R7}
    }
...
TEST_F(ThumbCPUTest15, LDMIA_SINGLE_REGISTER) {
    ...
    if (!assemble_and_write_thumb("ldmia r0!, {r1}", 0x00000000)) {
        memory.write16(0x00000000, 0xC802); // LDMIA R0!, {R1}
    }
    
    thumb_cpu.execute(1);
    
    // Check that data was loaded into R1
    EXPECT_EQ(cpu.R()[1], 0xDEADBEEFu);
    // Check that R0 was incremented
    EXPECT_EQ(cpu.R()[0], 0x00001004u);
    EXPECT_EQ(cpu.R()[15], 0x00000002u);
    ...
    if (!assemble_and_write_thumb("ldmia r2!, {r0}", 0x00000002)) {
        memory.write16(0x00000002, 0xCA01); // LDMIA R2!, {R0}
    }
    ...
    // Test case 3: LDMIA R7!, {R7}
    setup_registers({{7, 0x00001200}});
    memory.write32(0x00001200, 0xFEDCBA98);
    
    if (!assemble_and_write_thumb("ldmia r7!, {r7}", 0x00000004)) {
        memory.write16(0x00000004, 0xCF80); // LDMIA R7!, {R7}
    }
...
TEST_F(ThumbCPUTest15, LDMIA_MULTIPLE_REGISTERS) {
    ...
    if (!assemble_and_write_thumb("ldmia r0!, {r0, r1}", 0x00000000)) {
        memory.write16(0x00000000, 0xC803); // LDMIA R0!, {R0, R1}
    }
    
    thumb_cpu.execute(1);
    
    // R0 gets loaded with data, then the writeback occurs
    EXPECT_EQ(cpu.R()[0], 0xAAAAAAAAu); // R0 loaded first, then incremented
    EXPECT_EQ(cpu.R()[1], 0xBBBBBBBBu); // R1 loaded second
    EXPECT_EQ(cpu.R()[15], 0x00000002u);
    ...
    if (!assemble_and_write_thumb("ldmia r3!, {r1, r3, r5, r7}", 0x00000002)) {
        memory.write16(0x00000002, 0xCBAA); // LDMIA R3!, {R1, R3, R5, R7}
    }
    
    cpu.R()[15] = 0x00000002;
    thumb_cpu.execute(1);
    
    EXPECT_EQ(cpu.R()[1], 0x11111111u); // R1
    EXPECT_EQ(cpu.R()[3], 0xCCCCCCCCu); // R3 gets loaded value
    EXPECT_EQ(cpu.R()[5], 0x55555555u); // R5
    EXPECT_EQ(cpu.R()[7], 0x77777777u); // R7
    EXPECT_EQ(cpu.R()[15], 0x00000004u);
    
    // Test case 3: LDMIA R1!, {R0-R7}
    setup_registers({{1, 0x00001400}});
    for (int i = 0; i < 8; i++) {
        memory.write32(0x00001400 + (i * 4), 0x20000000 + i);
    }
    
    if (!assemble_and_write_thumb("ldmia r1!, {r0, r1, r2, r3, r4, r5, r6, r7}", 0x00000004)) {
        memory.write16(0x00000004, 0xC9FF); // LDMIA R1!, {R0-R7}
    }
...
TEST_F(ThumbCPUTest15, EMPTY_REGISTER_LIST) {
    // Test case 1: STMIA with empty register list
    setup_registers({{0, 0x00001000}});
    
    // Manual encoding for empty list (may not be valid assembly)
    memory.write16(0x00000000, 0xC000); // STMIA R0!, {}
    
    thumb_cpu.execute(1);
    
    // With empty register list, behavior is implementation defined
    // Some implementations don't modify the base register
    EXPECT_EQ(cpu.R()[0], 0x00001000u); // R0 unchanged
    EXPECT_EQ(cpu.R()[15], 0x00000002u);
    
    // Test case 2: LDMIA with empty register list
    setup_registers({{2, 0x00001100}});
    
    memory.write16(0x00000002, 0xCA00); // LDMIA R2!, {}
    
    cpu.R()[15] = 0x00000002;
    thumb_cpu.execute(1);
    
    // With empty register list, behavior is implementation defined
    EXPECT_EQ(cpu.R()[2], 0x00001100u); // R2 unchanged
    EXPECT_EQ(cpu.R()[15], 0x00000004u);
}
...
TEST_F(ThumbCPUTest15, MEMORY_ALIGNMENT_AND_BOUNDS) {
    ...
    EXPECT_EQ(cpu.R()[0], 0x1FF4u);
    EXPECT_EQ(cpu.R()[15], 0x00000002u);
    
    // Test load from the same location
    cpu.R()[1] = 0; // Clear R1
    
    if (!assemble_and_write_thumb("ldmia r0!, {r1}", 0x00000002)) {
        memory.write16(0x00000002, 0xC802); // LDMIA R0!, {R1}
    }
    
    cpu.R()[15] = 0x00000002;
    cpu.R()[0] = 0x1FF0; // Reset base address
    thumb_cpu.execute(1);
